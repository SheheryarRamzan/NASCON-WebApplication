...
insert into Category values ('1','CS','2','1');
...
------------CATEGORIES INSERTED BY DEFAULT-------------------
insert into category values('1','EE')
insert into category values('2','CS')
insert into category values ('3','Business')
insert into category values ('4','Social')
...
select * from sponsor
select * from studentexecutive
select categoryName, eventId, eventName, eventDate, eventVenue, eventTime, eventPrice from Category, Events, CategoryIncludes c where eventId = c.eventKey and categoryName='CS'



select mentorname from FacultyMentor f, CategorySuperviseByFaculty cf 
where f.mentorId=cf.mentorKey and cf.categoryKey=(select categoryId from Category where Categoryname='EE') 

select * from Participant where par_name='NATALIA'
select par_name from Participant p , regforfood r where p.par_cnic=r.participantkey
select eventname from events where eventDate=

select count(participantKey) as TotalParticipants from EventParticipatedBy 
where eventKey=(select eventid from events where eventName='EE');